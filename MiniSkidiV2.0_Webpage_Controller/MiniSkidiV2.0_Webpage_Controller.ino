...
<script>
    ...
    // THIS IS the value that adjusts the slider when buttons are pressed!
    const sliderAdjustmentLevel = 1

    const auxMinimum = 10
    const auxMaximum = 180
    ...
    function adjustBucket(value)
    {
        bucketSlider.value = Math.min(bucketMaximum, Math.max(bucketMinimum, parseInt(bucketSlider.value) + value))
        sendButtonInput("Bucket", bucketSlider.value)
    }

    function adjustAux(value)
    {
        auxSlider.value = Math.min(auxMaximum, Math.max(auxMinimum, parseInt(auxSlider.value) + value))
